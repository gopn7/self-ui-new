...
const testProps: AutoCompleteProps = {
  fetchSuggesition: (query) => {return testArray.filter(item => item.value.includes(query))},
  onSelect: jest.fn(),
...
describe('test AutoComplete component', () => {
  ...
  it('async fetchSuggestions should works fine', async () => {
    const testPropsWithPromise: AutoCompleteProps = {
      ...testProps,
      fetchSuggesition: jest.fn((query) => { return Promise.resolve(testArray.filter(item => item.value.includes(query))) }),
    ...
    fireEvent.change(inputNode, {target: { value: 'a'}})
    await waitFor(() => {
      expect(testPropsWithPromise.fetchSuggesition).toHaveBeenCalledWith('a')
      expect(wrapper.queryByText('ab')).toBeInTheDocument()
    })
  })
})